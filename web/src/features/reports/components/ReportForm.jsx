...
export default function ReportForm({ event, linkedParticipations = 0, onSave, onCancel }) {
  const [report, setReport] = useState({ feedbackAuthor: event.referent || '', feedbackTeam: event.referentTeam || '', ...(event.report || { attendance: event.participants, residents: linkedParticipations, revenue: event.revenue, costs: event.costs, feedback: '', outcomes: '' }) });
  ...
  return <form className="report-form" onSubmit={submit}>
    ...
    <div className="form-grid"><label className="field"><span>Responsable du retour *</span><input name="feedbackAuthor" value={report.feedbackAuthor} onChange={change} required /></label><label className="field"><span>Équipe / association du retour *</span><input name="feedbackTeam" value={report.feedbackTeam} onChange={change} required /></label><label className="field"><span>Présences totales *</span><input type="number" min="0" name="attendance" value={report.attendance} onChange={change} required /></label><label className="field"><span>Présences de résidentes *</span><input type="number" min="0" name="residents" value={report.residents} onChange={change} required /></label><label className="field"><span>Recettes réelles (€)</span><input type="number" min="0" step="0.01" name="revenue" value={report.revenue} onChange={change} /></label><label className="field"><span>Coûts réels (€)</span><input type="number" min="0" step="0.01" name="costs" value={report.costs} onChange={change} /></label><label className="field field-full"><span>Retour sur le déroulement *</span><textarea name="feedback" rows="4" value={report.feedback} onChange={change} placeholder="Ambiance, participation, points d’attention…" required /></label><label className="field field-full"><span>Résultats humains observés</span><textarea name="outcomes" rows="3" value={report.outcomes} onChange={change} placeholder="Liens créés, suites proposées, nouvelles opportunités…" /></label></div>
