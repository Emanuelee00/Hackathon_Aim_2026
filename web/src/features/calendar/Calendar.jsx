...
export default function Calendar({ onOpen }) {
  ...
  return <>
    <PageIntro eyebrow="GESTION DU LIEU" title="Le calendrier" description="Une vue claire des rencontres qui font vivre Chez Marthe.">
      <label className="calendar-filter"><span>Espace</span><select value={space} onChange={event => setSpace(event.target.value)}><option value="all">Tous les espaces</option>{bookableSpaces.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
    </PageIntro>
