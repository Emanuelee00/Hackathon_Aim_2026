  ...
  return <>
    <PageIntro eyebrow="LES LIEUX QUI NOUS RASSEMBLENT" title="Nos espaces" description="Trouvez le cadre juste pour chaque rencontre.">
      <div className="space-controls">
        <div className="site-tabs" role="tablist" aria-label="Antenne">{sites.map(candidate => <button key={candidate.id} type="button" className="chip" role="tab" aria-pressed={candidate.id === siteId} onClick={() => setSiteId(candidate.id)}>{candidate.label}</button>)}</div>
    ...
    {site?.capacity != null && <p className="date-chip site-gauge"><Icon name="users" size={14} /> Jauge globale du lieu, tous espaces confondus en simultané : {site.capacity} personnes</p>}
    <section className="space-overview" aria-label="Disponibilité des espaces"><div><strong>{siteSpaces.length}</strong><span>espaces à faire vivre</span></div><div><strong>{siteSpaces.length - occupied}</strong><span>disponibles ce jour</span></div><div><strong>{daily.length}</strong><span>réservations et demandes</span></div><div><strong>{siteSpaces.reduce((sum, space) => sum + (space.capacity || 0), 0)}</strong><span>capacité cumulée des salles (non simultanée)</span></div></section>
    <div className="spaces-grid">{siteSpaces.map(space => <SpaceCard key={space.id} space={space} siteName={site?.displayName} availability={availabilityFor(space.id, date, events)} onBook={spaceId => onBook(spaceId, date)} onOpen={onOpen} />)}</div>
  </>;
}
