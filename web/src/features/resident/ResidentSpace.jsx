...
function ProposalGroup({ title, hint, icon, items, eventById, onDecide, limit }) {
  ...
  return <section className="proposal-group"><h2><Icon name={icon} size={19} />{title}<small>{hint}</small></h2>
    {shown.map(match => <ProposalCard key={match.id} match={match} event={eventById[match.eventId]} onDecide={onDecide} />)}
...
export default function ResidentSpace({ navigate }) {
  ...
  const { fromTeam, fromCv, open } = groupProposals(mine, available, residentId);
  ...
  return <div className="resident-space">
    ...
    <section id="panel-proposals" role="tabpanel" aria-labelledby="tab-proposals" hidden={tab !== 'proposals'} className="resident-panel">
      ...
      <ProposalGroup title="Grâce à votre CV" hint="Trouvées en lisant votre CV" icon="sparkles" items={fromCv} eventById={eventById} onDecide={decide} />
      <ProposalGroup title="Ouvertes à toutes" hint="Activités à venir chez Marthe" icon="calendar" items={open} eventById={eventById} onDecide={decide} limit={2} />
      {!fromTeam.length && !fromCv.length && !open.length && <div className="resident-empty"><h2>Pas de nouvelle proposition</h2><p>L’équipe vous proposera des activités selon vos envies. En attendant, préparez votre plan emploi.</p><button className="button button-dark" onClick={() => setTab('plan')}>Préparer mon plan emploi<Icon name="arrow" size={17} /></button></div>}
    </section>
