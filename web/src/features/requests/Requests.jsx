import { compareRequests } from './queue.js';
import { requestTypes } from './workflow.js';
...
const filters = [
  ['all', 'Toutes'], ['pending', 'À étudier'], ['incomplete', 'Incomplètes'], ['waitlisted', 'Liste d’attente'], ['confirmed', 'Confirmées'],
...
export default function Requests({ onNew, onOpen }) {
  ...
  const shown = useMemo(() => events.filter(event => {
    ...
    return (requestType === 'all' || (requestType === 'unclassified' ? !event.requestType : event.requestType === requestType)) && matchesStatus && text.includes(query.trim().toLowerCase());
  }).sort(compareRequests), [events, query, status, requestType]);
  ...
  return <>
    ...
    <section className="panel requests-page">
      <div className="request-tools"><label className="field"><span>Type de demande</span><select value={requestType} onChange={e => setRequestType(e.target.value)}><option value="all">Tous les types</option>{Object.entries(requestTypes).map(([value, label]) => <option key={value} value={value}>{label}</option>)}<option value="unclassified">À préciser</option></select></label>
