import { priorities } from '../../features/requests/queue.js';
import { requestTypes } from '../../features/requests/workflow.js';
...
export default function EventRow({ event, onOpen, compact = false }) {
  ...
  return <button className={`event-row ${compact ? 'compact' : ''}`} onClick={() => onOpen(event.id)}>
    ...
    <span className="event-row-main"><strong>{event.title}</strong><span>{event.organizer}{event.priority ? ` · Priorité ${priorities[event.priority]}` : ''} · {requestTypes[event.requestType] || 'Type à préciser'}{event.status === 'pending' && event.reviewStage === 'committee' ? ' · En comité' : ''}</span><small className="mobile-event-date">{shortDate(event.date)} · {event.start}</small></span>
